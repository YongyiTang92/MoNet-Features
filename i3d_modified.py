...
class Unit3D(snt.AbstractModule):
    ...
    def _build(self, inputs, is_training):
        ...
        if self._use_batch_norm:
            bn = snt.BatchNormV2(scale=False, fused=False)
...
class InceptionI3d(snt.AbstractModule):
    ...
    def _build(self, inputs, is_training, dropout_keep_prob=1.0):
        ...
        end_point = 'Logits'
        with tf.variable_scope(end_point):
            feature_before_avg = tf.nn.avg_pool3d(net, ksize=[1, 1, 7, 7, 1],
        ...
        end_points['feature_before_avg'] = feature_before_avg
    ...
    def _resume_checkpoint(self, scope=None):
        variable_map = {}
        for variable in tf.global_variables():
            if scope is not None:
                ...
                variable_map[variable.name.replace(':0', '')] = variable
        self.saver = tf.train.Saver(var_list=variable_map, reshape=True)

    def _pretrain_init(self, sess, init_checkpoint_path, scope=None):
        if self.saver is not None:
            self.saver.restore(sess, init_checkpoint_path)
            logging.info("Restored model from " + init_checkpoint_path.split('/')[-1] +" .")
        else:
            self._resume_checkpoint(scope=scope)
            self.saver.restore(sess, init_checkpoint_path)
            logging.info("Restored model from " + init_checkpoint_path.split('/')[-1] +" .")
